...
export function ChatWorkspace({ currentUserId, initialPeerId }: { currentUserId: string; initialPeerId?: string }) {
  ...
  useEffect(() => {
    void fetch("/api/users").then((r) => r.json()).then((data) => {
      ...
      setUsers(list);
      if (initialPeerId && list.some((u: User) => u.id === initialPeerId)) setPeerId(initialPeerId);
      else if (list[0]?.id) setPeerId(list[0].id);
    });
  }, [initialPeerId]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      ...
      const pList = Array.isArray(pData) ? pData : (pData.presence || []);
      const tList = Array.isArray(tData) ? tData : (tData.typing || []);
      const pMap: Record<string, boolean> = {};
      for (const p of pList) pMap[p.userId] = Boolean(p.online);
      setPresence(pMap);
      setTypingKeys(tList.map((t: { key: string }) => t.key));
    }, 2500);
    return () => {
      clearInterval(timer);
      void fetch("/api/realtime/presence", {
            ...
            <form
              ...
              onSubmit={async (e) => {
                ...
                await fetch("/api/messages", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ recipientId: peerId, text: content.trim(), expiresMinutes: Number(ttl) > 0 ? Number(ttl) : undefined }),
                });
                setContent("");
                setSending(false);
                const res = await fetch(`/api/messages?peerId=${peerId}`);
