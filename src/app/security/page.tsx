...
export default async function SecurityPage() {
  ...
  // Retrieve security telemetry from the database
  ...
  // Calculate live security score based on active security preferences/events
  let securityScore = 60;
  if (user.id) securityScore += 10; // Registered & authenticated
  if (deviceCount > 0) securityScore += 15; // Has trusted device signature
  if (failedOtpCount === 0) securityScore += 15; // Clean login audit records
  if (messageCount > 0) securityScore += 10; // Encrypted persists active
